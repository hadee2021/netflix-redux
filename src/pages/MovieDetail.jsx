  ...
  return (
    <div className='center movie-detail-page'>
      <div className='poster-info-area'>
        <div>
          <img width={450} src={`https://image.tmdb.org/t/p/original///${detail.poster_path}`} />
        </div>
        <div>
          <div>
          ...
          <h2 className='movie-detail-div-layout'>{detail.title}</h2>
          ...
          <div className='movie-detail-rating-area'>
            <Rating
              ...
              readOnly 
            />
            <span className='item-badge'>{detail.vote_average} score</span>
          </div>
          <div className='movie-detail-div-layout'>
          ...
          {open && <TrailerModal setOpen={setOpen} videoArr={video.results} />}
        </div>
      </div>
          ...
          {tab === 'REVIEWS' && 
            <div className='review-area'>
              {reviews.results?.map(review => (
                <div key={review.id} className='review-area-item'>{review.content}</div>
              ))}
            </div>
          }
          {tab === 'RELATED MOVIES' &&
